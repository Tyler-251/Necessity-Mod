...
public class ElectrumHoodSetBonusBuff extends SetBonusBuff {
    public ElectrumHoodSetBonusBuff() {
        this.shouldSave = false;
    }

    public void init(ActiveBuff buff) {
        buff.setModifier(BuffModifiers.RANGED_ATTACK_SPEED, 0.15F);
    }

    public void onAttacked(ActiveBuff buff, MobHitEvent hitEvent) {
        super.onAttacked(buff, hitEvent);
        if (!hitEvent.isPrevented() && hitEvent.damageType == GameDamage.DamageType.RANGED) {
    ...
    public ListGameTooltips getTooltip(ActiveBuff ab) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "electrumhoodset"));
        return tooltips;
    }
}
