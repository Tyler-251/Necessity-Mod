...
public class HeavenRuinsCavePreset extends Preset {
    public HeavenRuinsCavePreset() {
        super(6, 10);
        ...
        this.fillObject(1,1,4, 8, ObjectRegistry.getObjectID("air"));
        ...
        for (int x = 0; x < 6; x++) {
            for (int y = 0; y < 10; y++) {
                ...
                if (getTile(x, y) == TileRegistry.getTileID("cloudstoneruinstile") && rand < 20) {
                    this.fillTile(x,y,1,1, TileRegistry.getTileID("darkcloudtile"));
                }
